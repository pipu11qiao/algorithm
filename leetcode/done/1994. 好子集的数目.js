...
var numberOfGoodSubsets = function (nums) {
  ...
  let len = Object.keys(allPrimeMap).length;
  if (len === 0) {
    return res
  }
  const dp = [];
    ...
    originArr.forEach(item => {
      Object.keys(selectMap).forEach(num => {
        ...
        if (key.every(_num => item.key.indexOf(_num) === -1)) {
          ...
          let numArr = [...item.num, Number(num)].sort((a, b) => a - b);
          let keyStr = numArr.join(',')
          if (!allMap[keyStr]) {
            dpArr.push({
              num: numArr,
              key: [...item.key, ...key].sort((a, b) => a - b),
              count
            });
            res += count % BASENUM
            allMap[keyStr] = 1
  ...
  for (let i = 0; i < len; i++) {
    ...
    // fn-1 map1
    (dp[i - 1] && dp[i - 1].length > 0) && select(dp[i - 1], curSinglePrimeMap, singlePrimeMap, dp[i]);
    // fn-2 map2
    (dp[i - 2] && dp[i - 2].length > 0) && select(dp[i - 2], curTwoPrimeMap, twoPrimeMap, dp[i]);
    // fn-3 map3
    (dp[i - 3] && dp[i - 3].length > 0) && select(dp[i - 3], curThreePrimeMap, threePrimeMap, dp[i]);
    if (i === 0 || i === 1 || i === 2) {
      let curMap = curSinglePrimeMap;
      let originMap = singlePrimeMap;
      if (i === 1) {
  ...
  console.log(`res`, res);
  if (oneCount > 0) {
    let oneSelectCount = getOneCount(oneCount);
    console.log(`oneSelectCount`, oneSelectCount);
    res += res * oneSelectCount;
    // res = (res % BASENUM) + ((res * oneSelectCount) % (BASENUM))
  }
