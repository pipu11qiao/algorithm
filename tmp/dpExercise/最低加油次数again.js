...
var minRefuelStops = function (target, startFuel, stations) {
  ...
  let dp = { 0: startFuel };
  for (let i = 1; i <= len; i++) {
    const curStation = stations[i - 1];
    const curFuel = curStation[1];
    const curD = curStation[0];
    let start = min === baseCount ? i : (min - 1)
    if (dp[start - 1] < curD) {
      break
    }
    for (let j = start; j > 0; j--) {
      if (dp[j - 1] >= curD) {
        curRes = Math.max(dp[j - 1] + curFuel, (dp[j] || 0))
        if (curRes >= target) {
          min = Math.min(j, min);
        }
        dp[j] = curRes
